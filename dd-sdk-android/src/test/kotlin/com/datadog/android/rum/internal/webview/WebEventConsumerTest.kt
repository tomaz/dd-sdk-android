...
internal class WebEventConsumerTest {
    ...
    fun `M delegate to RumEventConsumer W consume() { when RUM eventType }`(forge: Forge) {
        ...
        val fakeRumEventType = forge.anElementFrom(WebRumEventConsumer.rumEventTypes)
    ...
    fun `M delegate to LogsEventConsumer W consume() { LOG eventType }`(forge: Forge) {
        // Given
        val fakeBundledEvent = forge.getForgery<JsonObject>()
        val fakeLogEventType = forge.anElementFrom(WebLogEventConsumer.logEventTypes)
    ...
    fun `M do nothing W consume() { unknown event type }`(forge: Forge) {
        ...
        while (fakeUnknownEventType in WebRumEventConsumer.rumEventTypes ||
            fakeUnknownEventType in WebLogEventConsumer.logEventTypes
        ) {
